...
export async function generateText(opts: {
  ...
  try {
    ...
    throw new Error(`Impossible de joindre l'API Gemini : ${e instanceof Error ? e.message : String(e)}`);
  }
  ...
  if (!res.ok) {
    ...
    if (res.status >= 500)
      throw new Error(`Service Gemini indisponible (${res.status}), réessayez dans un instant.`);
    throw new Error(`Erreur Gemini (${res.status}) : ${body}`);
  }
